...
# ENTREGA 2:
    ...
    def ensamblar_sistema(self):
        ...
        for b in self.barras:
            ni= b.ni
            nj= b.nj
            # d, vector que condiciona los gdl globales en base a nodos ni y nj
            # Mapeo del elemento local al global
            d= np.array ([2*ni, ((2*ni)+1), 2*nj, ((2*nj)+1)])

            # Se agrega ese elemento a la ecuacion de rigidez global:
            for i in range (len(d)):
                ...
                for j in range(len(d)):
                    ...
                    # Componente ij se le suma a la matriz de rigidez 
                    self.k[p,q]+=ke[i,j]
                    
                # Fuerza de cada elemento
                    fe= b.obtener_vector_de_cargas(self)
                # Ensamblaje elementos: 
                self.f[p]+=fe[j]
